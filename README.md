  # **RomixerX Server** 🕯
  ### Мистический сервер майнкрафт с квестами и атмосферой приключений

  [![Discord](https://img.shields.io/discord/972218989235298385?color=5865F2&label=Discord&logo=discord&logoColor=red)](https://discord.com/invite/PNp3S3sanv)
  [![Website](https://img.shields.io/badge/romixerx.ru-online-111111?logoColor=white)](https://romixerx.ru)
  [![Telegram](https://img.shields.io/badge/Telegram-@RomixerX-26A5E4?logo=telegram&logoColor=red)](https://t.me/RomixerX)
  [![Rules](https://img.shields.io/badge/Rules-romixerx.ru-8B0000?logo=bookstack&logoColor=red)](https://romixerx.ru/rules)
  [![Support](https://img.shields.io/badge/Support-Telegram-111111?logo=telegram&logoColor=red)](https://t.me/romixsupport_bot)

  ---

  ## 🌑 **О сервере**

  **RomixerX** — мистический сервер с квестами, историей и атмосферой приключений.

  Исследуй мир загадок и мистики.  
  Выполняй квесты, зарабатывай валюту и раскрывай тайны сервера.

  **Версия:** Beta 0.3.1  
  **Поддержка:** 1.21.3 – 26.x

  ---

  ## ✨ **Особенности**

  | Категория | Описание |
  |:---------:|:---------|
  | 🧙 **Квесты** | 10 NPC с уникальными заданиями |
  | 🏰 **Мир** | Авторский спавн, лобби и арена |
  | 🎁 **Бонусы** | Бесплатные награды за квесты |
  | 👁 **Мистика** | Херобрин и мистические события |
  | 💰 **Экономика** | Две валюты и торговля |
  | ⚔ **Боссы** | Босс на `/warp arena` |
  | 💎 **Лут** | Осколки со случайным спавном |

  ---

  ## 👁️ **Скриншоты**

<div align="center">

| /menu | /mistika | /eenc list | /warp camp |
|:--:|:--:|:--:|:--:|
| ![Menu](https://cdn.modrinth.com/data/cached_images/0ba016b938ffbed97aea2686fa39ffbd86c90a5a.png) | ![mistika](https://cdn.modrinth.com/data/cached_images/c1088cdebc1283b6cd9744639995dba14bfbb320.png) | ![eenc](https://cdn.modrinth.com/data/cached_images/cfe0af16012788807876dbc9e6d76507d7098dae.png) | ![camp](https://i.ibb.co/jkGp4FGZ/2026-04-03-16-21-14.png) |

</div>

  ## 💰 **Экономика**

  ### Ромиксы — основная валюта

  Получение:

  - `/buyer`
  - Квесты

  Использование:

  - Обмен на Ромы
  - `/ah`
  - Казино `/warp casino`
  - Магазины игроков
  - Аренда регионов
  - Мифические торговцы
  
  ---

  ### Ромы — премиум валюта

  Получение:

  - Обмен Ромиксов
  - Сайт проекта: romixerx.ru

  Команда обмена:

  `/exchange`

  Ромы открывают доступ к эксклюзивным предметам.

  ---

  ## 🎁 **Бонусы**

  Получите **Ключ с Вещами** — просто введите ник на сайте.

  | Команда | Описание |
  |--------|----------|
  | `/kit` | Меню наборов |
  | `/free` | Ключи за время игры |

  ---

  ## 📊 **Статистика сервера**

  | Параметр | Значение |
  |:--------:|:--------:|
  | 📜 Квесты | 10+ |
  | 🧙 NPC | 10 |
  | 💰 Валюты | 2 |
  | 👁 Мистика | Есть |
  | ⚔ Боссы | Есть |

  ---

  ## 🛠 **Поддержка**

  Нужна помощь?

  - Telegram: https://t.me/RomixerX
  - Support Bot: https://t.me/romixsupport_bot

  ---

  ## ⭐ **Помоги проекту**

  Если тебе нравится сервер:

  - ⭐ Поддержи проект
  - 📢 Расскажи друзьям
  - 🎥 Делай видео
  - 💬 Вступай в Telegram

  ---

  <div align="center">

  **Удачи тебе в мистических приключениях, путник**

  </div>
